...
def read_graph(source: Path, sep=None, skip=0, directed=False, convert=int, weight_convert=int,
    ...
    Returns a list of nodes and a dict of edge weights"""
...
def write_edges(output_file: Path, edges: dict, mapper: dict):
    ...
    with open(output_file, "w") as dest:
        logging.info(f"Writing formatted edgelist to {output_file.resolve()}")
...
def main(args):
    ...
    if len(prefix) == 0 and len(suffix) == 0:
        logging.warning(f"No prefix or suffix entered; this will overwrite the original file!")
...
This python script is an attempt to reverse-engineer the `work.sh` file.""",
    ...
    ap.add_argument("-p", "--prefix", default="",
                    help="Output file will be named [prefix]_[file]_[suffix].[ext]. Default prefix is ''. Underscore is optional, and won't be included if prefix is empty.")
    ap.add_argument("-s", "--suffix", default="",
                    help="Output file will be named [prefix]_[file]_[suffix].[ext]. Default suffix is ''. Underscore is optional, and won't be included if suffix is empty.")
